...
class SettingsForm(FlaskForm):
    ...
    timeToWaitAfterAnomaliesUnusualIp=IntegerField('Time to wait after anomalies unusual ip', validators=[DataRequired(), NumberRange(min=1, max=100)], default=config["IDS_settings"]["TimeToWaitAfterAnomalies"].get("unusual_ip", 60), render_kw={"placeholder": "Time to wait after anomalies unusual ip (default = 60)"})
    timeToWaitAfterAnomaliesDnsTunneling=IntegerField('Time to wait after anomalies dns tunneling', validators=[DataRequired(), NumberRange(min=1, max=100)], default=config["IDS_settings"]["TimeToWaitAfterAnomalies"].get("dns_tunneling", 60), render_kw={"placeholder": "Time to wait after anomalies dns tunneling (default = 60)"})
    timeToWaitAfterAnomaliesMaliciousPayload=IntegerField('Time to wait after anomalies dns tunneling', validators=[DataRequired(), NumberRange(min=1, max=100)], default=config["IDS_settings"]["TimeToWaitAfterAnomalies"].get("dns_tunneling", 60), render_kw={"placeholder": "Time to wait after anomalies dns tunneling (default = 60)"})
    timeToWaitAfterAnomaliesArpSpoofing=IntegerField('Time to wait after anomalies arp spoofing', validators=[DataRequired(), NumberRange(min=1, max=100)], default=config["IDS_settings"]["TimeToWaitAfterAnomalies"].get("arp_spoofing", 60), render_kw={"placeholder": "Time to wait after anomalies arp spoofing (default = 60)"})
